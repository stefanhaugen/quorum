...
def test_hapi_feature_block_rejects_unknown_features():
    fs = FoodSecurityBlock(data=pd.DataFrame({
        "iso3": ["GTM"], "year": [2021], "ipc_phase": ["3+"],
        "population_in_phase": [1.0], "population_fraction_in_phase": [0.1],
    }))
    fp = FoodPriceBlock(data=pd.DataFrame({
        "iso3": ["GTM"], "year": [2021],
        "commodity_name": ["Maize"], "price": [0.5],
    }))
    df = pd.DataFrame({
        "iso3": ["GTM"], "year": [2021], "unrelated_column": [1.0],
    })
    errors = HAPIFeatureBlock(
        data=df, source_food_security=fs, source_food_prices=fp
    ).validate()
